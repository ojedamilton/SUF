<template>
    <main class="container pt-2">
        <div class="card">
            ...
            <div class="card-body table-responsive p-1">
                <table class="table table-striped table-valign-middle" id="compraTable">
                    <thead>
                        <tr>
                            <th>N° Compra</th>
                            <th>Comprador</th>
    ...
    methods: {
        listarCompras() {
            ...
            .then(function (response) {
                ...
                me.isLoading = false;
                const dataTable = $('#compraTable').DataTable({
                    searchPanes: {
                        cascadePanes: true,
                        viewTotal: true,
                        dtOpts:{
                    ...
                    "order": [[ 0, "desc" ]],
                    // Data para la tabla api/allcompras
                    "data": me.arrayCompras,
                    ...
                    columns: [
                        ...
                        { data: null,render(data)
                            {   return data.nombreProveedor+' '+data.apellidoProveedor} },
                        { data: "totalCompra" },
                        { data: "fechaCompra" },
                    ],
        ...
        async listarEmpresa(){
            ...
            return 'error Tipo Compra'
            }
        },
        listarDetallesById(id) {
            let me = this;
            var url = "/detallescomprasbyid";
            axios
            .post(url,{id:id})
            .then(function (response) {
                var respuesta = response.data;
  ...
  .compra > thead > tr > th:nth-child(2), .compra > tbod > tr > td:nth-child(2) {
    width: 300px;
  }
  
  .compra > thead > tr > th:nth-child(n+3) {
    text-align: right;
  }
  
  .compra > tbody > tr > td:nth-child(n+3) {
    text-align: right;
  }
  
  .compra > tfoot > tr > th, .compra > tfoot > tr > th:nth-child(n+3) {
    font-size: 19px;
    text-align: right;
  }
